...
function resolveWhatsAppUrl(number: string, message: string) {
...
export default function WhatsAppButton({ initialNumber = '', initialMessage = '' }: Props) {
  const [href, setHref] = useState(() => resolveWhatsAppUrl(initialNumber, initialMessage));

  useEffect(() => {
    getSettings()
      .then((res) => {
        const url = resolveWhatsAppUrl(res.data.whatsappNumber, res.data.whatsappMessage);
        setHref(url);
      })
      .catch(() => {
        setHref(resolveWhatsAppUrl(initialNumber, initialMessage));
      });
