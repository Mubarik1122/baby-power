        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`flex items-center justify-between transition-all duration-300 ${scrolled ? 'h-14 lg:h-16' : 'h-16 lg:h-[76px]'}`}>
            ...
            <nav className="hidden lg:flex items-center gap-7 ml-10 flex-1">
              ...
              >
                <button className="flex items-center gap-1 text-[13px] text-secondary hover:text-primary transition-colors duration-300">
                  Catalog
                  <ChevronDown className={`w-3.5 h-3.5 transition-transform duration-300 ${catalogOpen ? 'rotate-180' : ''}`} />
                </button>
                <div className={`absolute top-full left-0 pt-3 w-72 transition-all duration-300 ${catalogOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
                  <div className="bg-surface border border-border shadow-[0_16px_48px_-12px_rgba(44,44,44,0.15)] py-2">
                    <p className="px-5 py-2 text-[9px] uppercase tracking-[0.25em] text-muted">Collections</p>
                    <Link href="/shop" className="block px-5 py-2.5 text-sm text-secondary hover:bg-linen hover:text-primary transition-colors">
                      All Products
                    </Link>
                    {mounted && categories.map((cat) => (
                      <Link
                        key={cat._id}
                        href={`/shop/${cat.slug}`}
                        className="block px-5 py-2.5 text-sm text-muted hover:bg-linen hover:text-primary transition-colors"
                      >
                        {cat.name}
                      </Link>
                    ))}
      ...
      {open && (
        <div className="lg:hidden bg-surface border-b border-border shadow-lg">
          <nav className="px-4 py-4 space-y-1 max-h-[70vh] overflow-y-auto">
            ...
            <p className="pt-3 pb-1 text-[9px] uppercase tracking-[0.25em] text-muted">Catalog</p>
            <Link href="/shop" className="block py-2.5 pl-3 text-sm text-secondary" onClick={() => setOpen(false)}>All Products</Link>
            {mounted && categories.map((cat) => (
              <Link key={cat._id} href={`/shop/${cat.slug}`} className="block py-2.5 pl-3 text-sm text-muted hover:text-primary" onClick={() => setOpen(false)}>
                {cat.name}
              </Link>
            ))}
            {navLinks.slice(1).map((link) => (
